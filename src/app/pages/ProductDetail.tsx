  ...
  return (
    <div className="pt-[140px] pb-24">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        {/* Breadcrumbs */}
        <nav className="flex items-center space-x-2 text-[12px] uppercase tracking-widest text-gray-400 mb-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24">
          ...
          <div className="space-y-4">
            <motion.div 
              ...
              className="aspect-[4/5] bg-gray-100 rounded-[32px] overflow-hidden"
            >
              <img src={product.image} alt={product.name} className="w-full h-full object-cover" />
            </motion.div>
            <div className="grid grid-cols-4 gap-4">
              {[...Array(4)].map((_, i) => (
                <div key={i} className={`aspect-square rounded-2xl overflow-hidden cursor-pointer border-2 transition-all ${i === 0 ? 'border-[#6D4C91]' : 'border-transparent hover:border-gray-200'}`}>
          ...
          <div className="flex flex-col">
            <div className="mb-8">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center space-x-2">
                  <p className="text-[#6D4C91] text-[14px] font-bold uppercase tracking-widest">{product.brand}</p>
                  <span className="text-gray-300">•</span>
                  <p className="text-gray-400 text-[14px] font-medium uppercase tracking-widest">{product.category}</p>
                </div>
              ...
              <h1 className="text-[42px] font-serif leading-tight mb-4">{product.name}</h1>
              <div className="flex items-center space-x-4 mb-6">
                {product.stock <= 5 ? (
                  <span className="text-[12px] text-red-600 font-bold bg-red-50 px-4 py-1.5 rounded-full uppercase tracking-tighter flex items-center">
                    <span className="w-1.5 h-1.5 bg-red-600 rounded-full mr-2 animate-pulse" />
                    Low Stock
                  </span>
                ) : (
                  <span className="text-[12px] text-green-600 font-bold bg-green-50 px-4 py-1.5 rounded-full uppercase tracking-tighter flex items-center">
                    <span className="w-1.5 h-1.5 bg-green-600 rounded-full mr-2" />
                    In Stock
                  </span>
                )}
              </div>
              <p className="text-[32px] font-bold">{formatPrice(product.price)}</p>
            </div>

            <div className="mb-10 p-6 bg-[#FDFBF7] rounded-[24px]">
              <p className="text-gray-600 text-[16px] leading-relaxed mb-6">{product.description}</p>
              <div className="grid grid-cols-2 gap-4">
                <div className="flex items-center space-x-3 text-[13px] font-medium">
                  <Truck className="w-5 h-5 text-[#6D4C91]" />
                  <span>Free Shipping over KES 5k</span>
                </div>
                <div className="flex items-center space-x-3 text-[13px] font-medium">
                  <ShieldCheck className="w-5 h-5 text-[#6D4C91]" />
            ...
            <div className="space-y-6">
              <div className="flex items-center space-x-6">
                <div className="flex items-center border border-gray-200 rounded-full p-2 h-[64px]">
                  <button 
                ...
                <button 
                  ...
                  className="flex-grow bg-[#1A1A1A] text-white h-[64px] rounded-full text-[14px] font-bold uppercase tracking-widest hover:bg-[#6D4C91] transition-all flex items-center justify-center space-x-3"
                >
                  <ShoppingBag className="w-5 h-5" />
                  <span>Add to Cart — {formatPrice(product.price * quantity)}</span>
                </button>
              </div>
              <p className="text-center text-[12px] text-gray-400 font-medium">Safe & Secure Checkout via M-Pesa & Card</p>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-gray-100 pt-16">
          <div className="flex space-x-12 border-b border-gray-100 mb-12 overflow-x-auto whitespace-nowrap">
            {['description', 'ingredients', 'how to use'].map(tab => (
              <button 
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`pb-6 text-[14px] font-bold uppercase tracking-widest transition-all relative ${activeTab === tab ? 'text-[#1A1A1A]' : 'text-gray-400'}`}
              >
                {tab}
          ...
          <div className="max-w-3xl">
            {activeTab === 'description' && (
              <div className="space-y-6">
                <h3 className="text-[24px] font-serif">What it is</h3>
                <p className="text-gray-600 leading-relaxed text-[17px]">{product.description}</p>
              </div>
            )}
            {activeTab === 'ingredients' && (
              <p className="text-gray-600 leading-relaxed text-[17px]">{product.ingredients}</p>
            )}
            {activeTab === 'how to use' && (
              <p className="text-gray-600 leading-relaxed text-[17px]">{product.usage}</p>
            )}
          </div>
        </div>
      </div>
